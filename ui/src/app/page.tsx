...
export default function Home() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      // Step 1: ask the backend for an invite code + a presigned S3 PUT URL.
      // No file bytes are sent here, just metadata.
      const { data } = await axios.post('/api/upload', {
        filename: file.name,
        contentType: file.type || 'application/octet-stream',
      });

      // Step 2: upload the raw file straight to S3, bypassing the backend
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <header className="text-center mb-12">
        <h1 className="text-4xl font-bold text-blue-600 mb-2">peershare</h1>
      ...
      <footer className="mt-12 text-center text-gray-500 text-sm">
        <p>peershare &copy; {new Date().getFullYear()} - Secure P2P File Sharing</p>
      </footer>
    </div>
  );
}
